...
//---------------------------------------------------------------------------------
	...
	for(i = 0; i < NUM_SPRITES; i++) {
		...
		sprites[i].width = 128;
		sprites[i].height = 128;
		sprites[i].x = ((640 / 2 - sprites[i].width  / 2 )) << 8;
		sprites[i].y = ((480 / 2 - sprites[i].height / 2 )) << 8 ;
		sprites[i].dx = 0x6F + 0x100;
...
//		for(i = 0; i < NUM_SPRITES; i++) {
			if ( PAD_ButtonsHeld(0) & PAD_BUTTON_LEFT && 
					(sprites[curSprite].x >= (1<<8)) )
				sprites[curSprite].x -= sprites[curSprite].dx;
			else if ( PAD_ButtonsHeld(0) & PAD_BUTTON_RIGHT && 
					(sprites[curSprite].x <= ((640 - sprites[curSprite].width )<<8)) )
				sprites[curSprite].x += sprites[curSprite].dx;

			if (PAD_ButtonsHeld(0) & PAD_BUTTON_UP)
				sprites[curSprite].image = 1;
			else if (PAD_ButtonsHeld(0) & PAD_BUTTON_DOWN)
				sprites[curSprite].image = 2;

			else sprites[curSprite].image = 0;
//			sprites[curSprite].y += sprites[curSprite].dy;
			
			//check for collision with the screen boundaries
/*			if(sprites[curSprite].x < (1<<8) || sprites[curSprite].x > ((640-32) << 8))
				sprites[curSprite].dx = -sprites[curSprite].dx;

			if(sprites[curSprite].y < (1<<8) || sprites[curSprite].y > ((480-32) << 8))
				sprites[curSprite].dy = -sprites[curSprite].dy;
*/
			drawSpriteTex( sprites[curSprite].x >> 8, sprites[curSprite].y >> 8, sprites[curSprite].width, sprites[curSprite].height, sprites[curSprite].image);
//		}
